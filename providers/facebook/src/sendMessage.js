...
module.exports = function(defaultFuncs, api, ctx) {
  function uploadAttachment(attachments, callback) {
    ...
    bluebird
      ...
      .catch(function(err) {
        log.error("[Facebook] Error in uploadAttachment", err);
        return callback(err);
      });
  }
  ...
  return function sendMessage(msg, threadID, callback) {
    ...
    function sendContent(isSingleUser) {
      ...
      if(threadIDType === "Array") {
        ...
        log.info("[Facebook] Sending message to multiple users: " + threadID);
      } else {
      ...
      defaultFuncs
        ...
        .catch(function(err) {
          log.error("[Facebook] ERROR in sendMessage --> ", err);
          return callback(err);
        });
    }
